...
int main(int argc, char *argv[]) {
  ...
  if (sigfillset(&myAction.sa_mask) <
      0) /*setup everything for the timer - only used for teardown */
  ...
  for (;;) /* Run forever */
  {
    ...
    recvMsgSize = recvfrom(sock, &currPacket, sizeof(currPacket),
                           0, /* receive GBN packet */
                           (struct sockaddr *)&gbnClntAddr, &cliAddrLen);
    currPacket.type = ntohl(currPacket.type);
    currPacket.length =
        ntohl(currPacket.length); /* convert from network to host byte order */
    ...
    if (currPacket.type == 4) /* tear-down message */
    {
      ...
      if (sendto(sock, &ackmsg, sizeof(ackmsg), 0,
        ...
        DieWithError("Error sending tear-down ack"); /* not a big deal-data
                                                        already rcvd */
      }
      alarm(7);
      while (1) {
        ...
        if (ntohl(currPacket.type) == 4) /* respond to more teardown messages */
        {
          ...
          ackmsg.seq_no = htonl(0); /* convert to network endianness */
          ackmsg.length = htonl(0);
          if (sendto(sock, &ackmsg, sizeof(ackmsg), 0, /* send teardown ack */
                     (struct sockaddr *)&gbnClntAddr,
                     cliAddrLen) != sizeof(ackmsg)) {
      ...
      if (currPacket.seq_no == packet_rcvd + 1) {
        ...
        memcpy(&buffer[buff_offset],
               currPacket.data, /* copy packet data to buffer */
               currPacket.length);
      }
      printf("---- SEND ACK %d\n", packet_rcvd);
...
void CatchAlarm(int ignored) /* just exit when encounter timeout */
{
  exit(0);
}
